...
Gaps and conflicts stay visible. They are not hidden. They are not filled in by guessing.

A model with many unknowns is still valid understanding.

Understanding does not wait for unknowns to be resolved.
...
This definition closes both gaps.

There is no separate step after modeling where Tend "determines the ask." The model is the determination.
...
If "understand" meant "ready to act," we would collapse understanding and deciding into one step.

Level 1 treats them as separate. This definition keeps them separate.
...
Product Vision lists questions like: was the order confirmed? was payment received? was it dispatched?

Those are not questions Tend must answer before understanding counts.

They are slots in the model.

At first, most of them are unknown.

Understanding names them. Gathering fills them.

Similarly, if two sources disagree, the model marks the conflict.

It does not pick a side silently.
...
The customer may state one problem. The real issue may be something else.

That divergence lives in the model as two separate layers.

Understanding records the divergence. It does not resolve it.

Resolving it — grounding facts, verifying beliefs — happens in gathering, the step after understanding.
...
- **Known** — we have this and treat it as established for now
- **Unknown** — we do not have this yet; the slot exists but is empty
- **Conflicting** — two sources disagree; neither side is silently chosen
...
Understanding does not decide the next step.

Replying, escalating, waiting, updating a system — those belong to decision — the step after gathering.

Understanding does not invent facts to fill empty slots.

Understanding does not hide conflicts by picking one source.
...
Understanding builds the model. It does not reply.

**What the customer is asking:** A delivery status update. They believe a delivery promise was broken.

**Customer context:** Who this person is. Their profile. Their history with the business.

**What we have already told them:** Maybe someone promised "yesterday" on WhatsApp three days ago. The customer is now writing on email.

**Known:** Customer says delivery was promised yesterday. Delivery has not arrived.

**Unknown:** Which order? Was it confirmed? Was payment received? Was it dispatched? Is there a delay? Did the delivery partner report a problem? Has another employee already spoken to them? Was a new date promised? Was a refund approved?
