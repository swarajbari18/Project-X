...
Easily, and mostly by an existing decision: a situation is one operational problem, and the same person can have many of them open at once. The situation graph lets one message attach to several situations, split a conversation into several problems, and link related-but-separate situations. The person-view is derived; it does not flatten these.
...
- One customer can have several situations open at the same time.
...
The new piece from Journey and Lifecycle is that the *person* (who has all these situations) is an identity-bearing anchor, and the person's relationship is derived from their situations.
